...
function watchFilter(glob) {
  return Array.isArray(glob)
    ? glob.filter((element) => !element.startsWith('!'))
    : glob;
}
...
gulp.task('duplicate:sub1', () => {
  return gulp
    .src(Cfg.duplicate.src, { since: gulp.lastRun('duplicate:sub1') })
    .pipe(gulpChanged(Cfg.duplicate.dest))
    .pipe(gulp.dest(Cfg.duplicate.dest));
});
gulp.task('duplicate:sub2', () => {
  return gulp
    .src(Cfg.duplicate.wpSrc, { since: gulp.lastRun('duplicate:sub2') })
    .pipe(gulpIf(Cfg.duplicate.wpFlg, gulpChanged('./src/wp/')))
    .pipe(gulpIf(Cfg.duplicate.wpFlg, gulp.dest('./src/wp/')));
});
...
gulp.task('html', () => {
  ...
  return (
    gulp
      ...
      .pipe(gulpIf(Cfg.html.minFlg, gulpHtmlmin({ collapseWhitespace: true })))
...
gulp.task('sass', () => {
  return (
    gulp
      ...
      .pipe(
        gulpPostcss(
          [stylelint(), postcssReporter({ clearReportedMessages: true })],
          {
            syntax: postcssScss,
          },
        ),
      ...
      // sassコンパイル
      // オプションで吐き出す形式を設定
      .pipe(gulpSass({ fiber: fibers, outputStyle: Cfg.scss.outputStyle }))
      .pipe(
        gulpPostcss([
          // prefixを自動付与
      ...
      .pipe(
        gulp.dest(Cfg.scss.dest, {
          sourcemaps: Cfg.scss.mapFlg ? Cfg.scss.sourceMaps : false,
        }),
      )
      .pipe(browserSync.reload({ stream: true }))
      .pipe(gulpIf(Cfg.scss.wpFlg, gulp.dest('./src/wp/css/')))
  );
});
...
gulp.task('js', () => {
  return gulp
    .src(Cfg.js.entry, { sourcemaps: Cfg.js.mapFlg })
    .pipe(
      gulpPlumber({
        errorHandler: gulpNotify.onError('<%= error.message %>'),
      }),
    ...
    .pipe(
      gulp.dest(Cfg.js.dest, {
        sourcemaps: Cfg.js.mapFlg ? Cfg.js.sourceMaps : false,
      }),
    )
    .pipe(browserSync.reload({ stream: true }))
    .pipe(gulpIf(Cfg.js.wpFlg, gulp.dest('./src/wp/js/')));
});

// images用タスク
gulp.task('image', () => {
  return gulp
    .src(Cfg.img.src, { since: gulp.lastRun('image') })
    .pipe(
      gulpPlumber({
        errorHandler: gulpNotify.onError('<%= error.message %>'),
      }),
    ...
    .pipe(browserSync.reload({ stream: true }))
    .pipe(gulpIf(Cfg.img.wpFlg, gulp.dest('./src/wp/img/')));
});
...
gulp.task(
  'default',
  gulp.parallel('duplicate', 'html', 'ejs', 'sass', 'js', 'image', 'watch'),
);
